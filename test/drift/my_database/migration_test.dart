...
void main() {
  ...
  test('migration from v1 to v2 does not corrupt data', () async {
    ...
    final oldBlocksData = <v1.BlocksData>[];
    final expectedNewBlocksData = <v2.BlocksData>[];
    ...
    await verifier.testWithDataIntegrity(
      ...
      createItems: (batch, oldDb) {
        ...
        batch.insertAll(oldDb.blocks, oldBlocksData);
      },
      validateItems: (newDb) async {
        expect(expectedNewEntriesData, await newDb.select(newDb.entries).get());
        expect(expectedNewBlocksData, await newDb.select(newDb.blocks).get());
      },
    );
  });
}
